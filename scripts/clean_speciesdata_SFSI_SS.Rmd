...
#or the clusters combined with the SFSI - 1.5, 5 % - species and genus level 
...
SFSI.SS.specraw.5<-read_delim("../raw_data/all_tag_by_cluster_counts6353.txt",delim = ";",col_names = TRUE,trim_ws = FALSE,show_col_types = FALSE)


datasetlist<-list(SFSI.SS.specraw.1,SFSI.SS.specraw.5)
...
```{r}
#make ecto only data set 
datasetlist.ecm.sfsi.genera<-datasetlist.SFSI[[2]][,which(colnames(datasetlist.SFSI[[2]]) %in% taxsetlist[[2]][which(taxsetlist[[2]]$Ecology == "ECM"),]$`Cluster ID`)]
#only 3 percent

#remove samples where there are no ectos...
datasetlist.ecm.sfsi.genera$JordProvnr<-datasetlist.SFSI[[2]]$tag1

datasetlist.ecm.sfsi.genera<-datasetlist.ecm.sfsi.genera[-which(rowSums(datasetlist.ecm.sfsi.genera[,-which(colnames(datasetlist.ecm.sfsi.genera) == "JordProvnr")]) == 0),]

datasetlist.rel.sfsi.ecm.genera<-datasetlist.ecm.sfsi.genera[,-which(colnames(datasetlist.ecm.sfsi.genera) == "JordProvnr")]/rowSums(datasetlist.ecm.sfsi.genera[,-which(colnames(datasetlist.ecm.sfsi.genera) == "JordProvnr")])

datasetlist.rel.sfsi.ecm.genera$JordProvnr<-datasetlist.ecm.sfsi.genera$JordProvnr
...
write_csv(datasetlist.rel.sfsi.genera,"../clean_data/SFSI_data_relabun_5perc.csv")
write_csv(datasetlist.rel.sfsi.ecm.genera,"../clean_data/SFSI_data_relabun_ECM_5perc.csv")
```

...
for (i in 1:length(datasetlist.p2)){
datasetlist.p2.agar[[i]]<-datasetlist.p2[[i]][,which(colnames(datasetlist.p2[[i]]) %in% taxsetlist.agar[which(taxsetlist.agar$Class == "Agaricomycetes"),]$`Cluster ID`)]
}
...
single.species<- datasetlist.rel.agar.genus[,which(colnames(datasetlist.rel.agar.genus) %in% only.1.species)] #83 genera
several.species<- datasetlist.rel.agar.genus[,-which(colnames(datasetlist.rel.agar.genus) %in% only.1.species)] # 671 species remaning
...
datasetlist.rel.agar.genus.combine[[i]] <- rowSums(several.species[,ind]) #this sums them and enters each as an item on the list
}
...
colnames(datasetlist.rel.agar.genus.combine)<-unique(colnames(several.species))
...
datasetlist.rel.agar.genus<-cbind(datasetlist.rel.agar.genus.combine,single.species)

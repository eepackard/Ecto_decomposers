...
sfsi.pine$fert.age.pca<-c(pca.sfsi.age.fert.pine$x[,1])
sfsi.spruce$fert.age.pca<-c(pca.sfsi.age.fert.spruce$x[,1])
...
pca.sfsi<-prcomp(sfsi.fungniche[,which(colnames(sfsi.fungniche) %in% c("fert.pca","age"))]) 

#tiff("../figures/PCA_age_NC_pH.tiff",width = 100,height = 100,res = 1000,units = "mm")
fviz_pca_biplot(pca.sfsi,axes = c(1,2),geom = "point",repel = T)#,invisible = "ind"
...
```{r}
lm.relecm<-lmer(miner.relecmlog~ age +fert.pca+ spruce+ (1|Seqplat),data = sfsi.pres)#
summary(lm.relecm)
summary(aov(miner.relecmlog~ age +fert.pca+spruce+  (1|Seqplat),data = sfsi.pres))#
plot(lm.relecm)
r.squaredGLMM(lmer(miner.relecmlog~ age +fert.pca+  (1|Seqplat),data = sfsi.pres))#spruce+

fert_resid <- resid(lm(data=sfsi.pres, fert.pca ~ age_log ))
age_resid <- resid(lm(data=sfsi.pres, age_log ~ fert.pca ))
#miners_resid <- resid(lm(data=sfsi.pres, miner.relecmlog ~  Seqplat))
ggplot(data=NULL) + geom_point(aes(x=age_resid, y=sfsi.pres$miner.relecmlog))
ggplot(data=NULL) + geom_point(aes(x=fert_resid, y=sfsi.pres$miner.relecmlog))
summary(aov(sfsi.pres$miner.relecmlog~ fert_resid+  (1|sfsi.pres$Seqplat)))
summary(aov(sfsi.pres$miner.relecmlog~ age_resid +  (1|sfsi.pres$Seqplat)))
...
```
```{r}
#linear model with relecmative abun of guild 
#pine
lm.rel.pine<-lmer(miner.rellog~ age +fert.pca+(1|Seqplat),data = sfsi.pine.pres)#
...
summary(lm.rel.pine)
summary(aov(miner.rellog~age +fert.pca+(1|Seqplat),data = sfsi.pine.pres))
...
r.squaredGLMM(lmer(miner.rellog~age+fert.pca+(1|Seqplat),data = sfsi.pine.pres))
r.squaredGLMM(lmer(miner.rellog~age_log+fert.pca+(1|Seqplat),data = sfsi.pine.pres))

#spruce
lm.rel.spruce<-lmer(miner.rellog~age+fert.pca+(1|Seqplat),data = sfsi.spruce.pres)#
...
summary(aov(miner.rellog~age+fert.pca+(1|Seqplat),data = sfsi.spruce.pres))#
...
r.squaredGLMM(lmer(miner.rellog~age+fert.pca+(1|Seqplat),data = sfsi.spruce.pres))
...
for (i in 1:6){
pearson.age.results.pine.ecm[[i]]<-cor.test(as.numeric(unlist(sfsi.pine[which(sfsi.pine[, col.num[[1]][i]] == "1"),which(colnames(sfsi.pine) == "unscaled_age")])),as.numeric(unlist(log(sfsi.pine[which(sfsi.pine[, col.num[[1]][i]] == "1"),which(colnames(sfsi.pine) == list.rel.miners.ecm[[1]][[i]])])))) 

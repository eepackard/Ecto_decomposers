...
#spruce
lm.relecm.spruce<-lmer(miner.relecmlog~age+fert.pca+(1|Seqplat),data = sfsi.spruce.pres)# change to lmer then add 
...
pearson.results.pine.ecm<-list()
pearson.results.spruce.ecm<-list()
for (i in 1:6){
...
pearson.results.pine.ecm[[i]]<-cor.test(as.numeric(unlist(sfsi.pine[which(sfsi.pine[, col.num[[1]][i]] == "1"),which(colnames(sfsi.pine) == "unscaled_age")])),as.numeric(unlist(log(sfsi.pine[which(sfsi.pine[, col.num[[1]][i]] == "1"),which(colnames(sfsi.pine) == list.rel.miners.ecm[[1]][[i]])])))) 

pearson.results.spruce.ecm[[i]]<-cor.test(as.numeric(unlist(sfsi.spruce[which(sfsi.spruce[, col.num[[1]][i]] == "1"),which(colnames(sfsi.spruce) == "unscaled_age")])),as.numeric(unlist(log(sfsi.spruce[which(sfsi.spruce[, col.num[[1]][i]] == "1"),which(colnames(sfsi.spruce) == list.rel.miners.ecm[[1]][[i]])])))) 
 }

wilcox.results.pine.ecm
wilcox.results.spruce.ecm
pearson.results.pine.ecm
pearson.results.spruce.ecm
```

##correct p_values
```{r}
p.value.pine.list.pearson<-list()
p.value.spruce.list.pearson<-list()
p.value.pine.list.wilcox<-list()
p.value.spruce.list.wilcox<-list()
p.value.pine.list.fisher<-list()
p.value.spruce.list.fisher<-list()

#extract p values
for( i in 1:6){
p.value.pine.list.pearson[[i]]<-pearson.results.pine.ecm[[i]]$p.value
p.value.pine.list.wilcox[[i]]<-wilcox.results.pine.ecm[[i]]$p.value
p.value.pine.list.fisher[[i]]<-fisher.results.pine[[i]]$p.value
p.value.spruce.list.pearson[[i]]<-pearson.results.spruce.ecm[[i]]$p.value
p.value.spruce.list.wilcox[[i]]<-wilcox.results.spruce.ecm[[i]]$p.value
p.value.spruce.list.fisher[[i]]<-fisher.results.spruce[[i]]$p.value
}

p.adjust(unlist(p.value.pine.list.pearson),method = "fdr")
p.adjust(unlist(p.value.pine.list.wilcox),method = "fdr")
p.adjust(unlist(p.value.pine.list.fisher),method = "fdr")
p.adjust(unlist(p.value.spruce.list.pearson),method = "fdr")
p.adjust(unlist(p.value.spruce.list.wilcox),method = "fdr")
p.adjust(unlist(p.value.spruce.list.fisher),method = "fdr")
```

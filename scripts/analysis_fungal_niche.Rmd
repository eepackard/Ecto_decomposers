...
pca.sfsi<-prcomp(SFSI.soil.sel[,which(colnames(SFSI.soil.sel) %in% c("Bestandsalder_scaled","pH_H2O_log_scaled","Temperatursumma_scaled","NC_scaled"))]) 
...
pca.sfsi.age.fert.pine<-prcomp(sfsi.pine[,which(colnames(sfsi.pine) %in% c("age","fert.pca"))]) 
pca.sfsi.age.fert.spruce<-prcomp(sfsi.spruce[,which(colnames(sfsi.spruce) %in% c("age","fert.pca"))])
fviz_pca_biplot(pca.sfsi.age.fert.pine,axes = c(1,2))#,invisible = "ind"
#fviz_pca_biplot(pca.sfsi.age.fert.pine,axes = c(1,3))#,invisible = "ind"
fviz_pca_biplot(pca.sfsi.age.fert.spruce,axes = c(1,2))#,invisible = "ind"
#fviz_pca_biplot(pca.sfsi.age.fert.spruce,axes = c(1,3))#,invisible = "ind"


sfsi.pine$fert.age.pca<-c(pca.sfsi.age.fert.temp.pine$x[,1])
sfsi.spruce$fert.age.pca<-c(pca.sfsi.age.fert.temp.spruce$x[,1])
...
```{r}
lm.relecm<-lmer(miner.relecmlog~ age +fert.pca+  (1|Seqplat),data = sfsi.pres)#spruce+
summary(lm.relecm)
summary(aov(miner.relecmlog~age +fert.pca+  (1|Seqplat),data = sfsi.pres))#spruce+
plot(lm.relecm)
r.squaredGLMM(lmer(miner.relecmlog~ age +fert.pca+  (1|Seqplat),data = sfsi.pres))#spruce+
...
lm.relecm.pine<-lmer(miner.relecmlog~fert.pca+agecat+(1|Seqplat),data = sfsi.pine.pres)
...
```{r}
#remove data point with forest 335 years?
#sfsi.pine.pres<-sfsi.pine.pres[-which(sfsi.pine.pres$unscaled_age >300),]

fert_resid <- resid(lm(data=sfsi.pine.pres, fert.pca ~ unscaled_age + temp + Seqplat ))
age_resid <- resid(lm(data=sfsi.pine.pres, unscaled_age ~ fert.pca + temp + Seqplat))
miners_resid <- resid(lm(data=sfsi.pine.pres, miner.relecmlog ~ temp + Seqplat))
ggplot(data=NULL) + geom_point(aes(x=age_resid, y=miners_resid))
ggplot(data=NULL) + geom_point(aes(x=fert_resid, y=miners_resid))
cor.test(miners_resid,age_resid)
cor.test(miners_resid,fert_resid)

fert_resid <- resid(lm(data=sfsi.spruce.pres, fert.pca ~ unscaled_age + temp + Seqplat ))
age_resid <- resid(lm(data=sfsi.spruce.pres, unscaled_age ~ fert.pca + temp + Seqplat))
miners_resid <- resid(lm(data=sfsi.spruce.pres, miner.relecmlog ~ temp + Seqplat))
ggplot(data=NULL) + geom_point(aes(x=age_resid, y=miners_resid))
ggplot(data=NULL) + geom_point(aes(x=fert_resid, y=miners_resid))
cor.test(miners_resid,age_resid)
cor.test(miners_resid,fert_resid)

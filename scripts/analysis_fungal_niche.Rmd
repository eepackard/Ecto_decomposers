...
#remove Suillus, Thelephora, ,sitotrema etc... 
...
sfsi.fungniche.all$fert.pca<-c(pca.sfsi.fert$x[,1]*-1)
```
...
```{r}
glm.pa<-glmer(miners.pres~age_log+I(age_log^2)+fert.pca+I(fert.pca^2)+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.fungniche.all)#+I(fert.pca^2)
glm.pa<-glmer(miners.pres~agecat+fert.pca+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.fungniche.all)#+I(fert.pca^2)
summary(glm.pa)
Anova(glm.pa)
r.squaredGLMM(glm.pa)

glm.pa.spruce<-glmer(miners.pres~age_log+fert.pca+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.spruce)#+Mn+I(age_log^2)+I(fert.pca^2)
glm.pa.spruce<-glmer(miners.pres~agecat+fert.pca+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.spruce)#+I(fert.pca^2)
summary(glm.pa.spruce)
Anova(glm.pa.spruce)
r.squaredGLMM(glm.pa.spruce)

glm.pa.pine<-glmer(miners.pres~age_log+I(age_log^2)+fert.pca+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.pine)#+I(fert.pca^2)
glm.pa.pine<-glmer(miners.pres~agecat+fert.pca+Mn_sqrt+(1|Seqplat),family = binomial(link = "logit"),data = sfsi.pine)
summary(glm.pa.pine)
Anova(glm.pa.pine)
...
```{r}
#pine
lm.relecm.pine<-lmerTest::lmer(miner.relecmlog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres,na.action = na.omit)#+ I(age_log^2)
summary(lm.relecm.pine)
lm.relecm.pine<-lmerTest::lmer(miner.relecmlog~age_log +fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres)
summary(lm.relecm.pine)
plot(lm.relecm.pine)
r.squaredGLMM(lmer(miner.relecmlog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres))
r.squaredGLMM(lmer(miner.relecmlog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres))

#spruce
lm.relecm.spruce<-lmerTest::lmer(miner.relecmlog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres)#
summary(lm.relecm.spruce)
lm.relecm.spruce<-lmerTest::lmer(miner.relecmlog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres)
summary(lm.relecm.spruce)
plot(lm.relecm.spruce)
r.squaredGLMM(lmer(miner.relecmlog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres))
r.squaredGLMM(lmer(miner.relecmlog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres))
...
```{r}
 #pine
lm.rel.pine<-lmerTest::lmer(miner.rellog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres)#
summary(lm.rel.pine)
lm.rel.pine<-lmerTest::lmer(miner.rellog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres)
summary(lm.rel.pine)
plot(lm.rel.pine)
r.squaredGLMM(lmer(miner.rellog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres))
r.squaredGLMM(lmer(miner.rellog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.pine.pres))

#spruce
lm.rel.spruce<-lmerTest::lmer(miner.rellog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres)#
summary(lm.rel.spruce)
lm.rel.spruce<-lmerTest::lmer(miner.rellog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres)
summary(lm.rel.spruce)
plot(lm.rel.spruce)
r.squaredGLMM(lmer(miner.rellog~agecat+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres))
r.squaredGLMM(lmer(miner.rellog~age_log+fert.pca+Mn_sqrt+(1|Seqplat),data = sfsi.spruce.pres))
...
list.pres.abs.miners.s<-list(c(colnames(sfsi.spruce[,21:30])))
list.pres.abs.miners.p<-list(c(colnames(sfsi.pine[,c(21:28,30)])))
list.pres.abs.miners<-list(c(colnames(sfsi.fungniche.all[,21:30])))
...
```{r}
list.pres.abs.miners.all<-list(c(colnames(sfsi.pres[,21:30])))
...
```{r}
#the only difference between the full soil dataset and the set used for data analysis is I remove samples that have 50% pine and spruce - even split so need to go from 958 to 929 samples
...
write_csv(SFSI.soil.sel[,c(14,34,20,23,24,26)],"../clean_data/SFSI_metadata_SRA.csv")
```

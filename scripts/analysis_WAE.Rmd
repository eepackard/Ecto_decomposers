...
#read in data
```{r}
datasetlist.rel.agar.15<-list(read_csv("../clean_data/SS_data_relabun_agar_15_1.5perc.csv"),read_csv("../clean_data/SS_data_relabun_agar_15_5perc.csv"))
...
wae.15<-as.data.frame(colSums(Meta.data.spatial$MnP_gOM_scaled*datasetlist.rel.agar.15[[1]])/colSums(datasetlist.rel.agar.15[[1]]))
colnames(wae.15)<-c("Weight.aver")

wae.15.genera<-as.data.frame(colSums(Meta.data.spatial$MnP_gOM_scaled*datasetlist.rel.agar.15[[2]])/colSums(datasetlist.rel.agar.15[[2]]))
...
permtest.results.15 <- permutation.test(MnP =  Meta.data.spatial$MnP_gOM_scaled, spec.data = datasetlist.rel.agar.15[[1]],test.stat = wae.15, 10000)
...
wae.perm.results.15<-as.data.frame(cbind(colnames(datasetlist.rel.agar.15[[1]]),p.adjust(permtest.results.15[[1]],method = "fdr",length(permtest.results.15[[1]])),permtest.results.15[[1]]))
wae.perm.results.15[which(wae.perm.results.15$V3 < 0.1),]
wae.perm.results.15$ID<-taxsetlist.agar[[1]][match(wae.perm.results.15$V1,taxsetlist.agar[[1]]$`Cluster ID`),]$Reference
...
permtest.results.15.genera <- permutation.test(MnP =  Meta.data.spatial$MnP_gOM_scaled, spec.data = datasetlist.rel.agar.15[[2]],test.stat = wae.15.genera, 10000)

wae.perm.results.15.genera<-as.data.frame(cbind(colnames(datasetlist.rel.agar.15[[2]]),p.adjust(permtest.results.15.genera[[1]],method = "fdr",length(permtest.results.15.genera[[1]])),permtest.results.15.genera[[1]]))
...
colnames(wae.perm.results.15.genera)<-c("Cluster_ID","adjusted_p","p","ID")
...
```
#get list of all species in clusters
```{r}
sorted.perm.results<-wae.perm.results.15[match(sort(wae.perm.results.15$p),wae.perm.results.15$p),]
sorted.perm.results

species_list<-sorted.perm.results[c(2,4,5,7,9,12),]$ID
```

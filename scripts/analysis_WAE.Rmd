...
abline(v=wae.15[which(rownames(wae.15)== "scata5689_66"),], lwd=3, col="red")

#plot suillus 
hist(as.numeric(as.data.frame(permtest.results.15[[2]])[70,]), breaks=50, col='grey', main="Permutation Distribution", las=1, xlab='')
abline(v=wae.15[which(rownames(wae.15)== "scata5689_294"),], lwd=3, col="red")
...
```
```{r}
which(rownames(scores(MnP.CCA,choices = 1)$species) %in% taxsetlist.agar[[1]][which(taxsetlist.agar[[1]]$Class == "Agaricomycetes"),]$`Cluster ID`) #length139


scores.MnP.agar<-as.data.frame(cbind(scores(MnP.CCA,choices = 1)$species,rownames(scores(MnP.CCA,choices = 1)$species)))


scores.MnP.agar<-scores.MnP.agar[which(scores.MnP.agar$V2 %in% taxsetlist.agar[[1]][which(taxsetlist.agar[[1]]$Class == "Agaricomycetes"),]$`Cluster ID`),]

...
SFSI.soil.f2$host<-if_else(SFSI.soil.f2$Granandel >= 60,"Spruce",if_else(SFSI.soil.f2$Tallandel >= 60,"Pine","miss")) 
SFSI.soil.f2<-SFSI.soil.f2[-which(SFSI.soil.f2$host == "miss"),]
```
...
map1 <- tm_shape(sverige_simp) +
  ...
  tm_dots(col = "host", size = 0.25,alpha=0.5,shape=19,palette=c("#AACB90","#415B25"),title = "National samples",legend.show = FALSE)+
  ...
  tm_add_legend(type = "symbol",shape =19 ,col = c("#AACB90","#415B25"),labels = c("Pine dominated", "Spruce dominated"),title = "Large-scale samples")+
...
map2 <- tm_shape(sverige_simp) +
  ...
  tm_add_legend(type = "symbol",shape=19,col = c("navy","dodgerblue"),labels = c("Old > 70 years", "Young < 70 years"),title = "Regional samples")+
  tm_layout(legend.width = 1,legend.outside = TRUE,legend.position = c(-0.15,0.1),frame = FALSE)
...
map4 <- tm_shape(sverige,bbox = c(xmin=600000,ymin=6603368,xmax=736000,ymax=6750000)) +
  tm_polygons(border.col = "black", col = "white", alpha = 0.1)+ 
  tm_shape(sjoar,bbox = c(xmin=600000,ymin=6603368,xmax=736000,ymax=6750000)) +
  tm_fill(col = "lightblue")+
  tm_shape(data_sf_ss["Species"]) +
  tm_dots(col = "Species", size = 4.0,border.lwd=3,alpha = 0.5,shape=19,palette=c("navy","dodgerblue"),title = "Regional samples",legend.show = FALSE)
...
tmap_save(tm = map2, filename = "../figures/Age_map.pdf")
tmap_save(tm = map4, filename = "../figures/Age_map_inset.pdf")
```
